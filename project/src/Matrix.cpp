...
template<size_t rows, size_t cols>
double*  Matrix<rows, cols>::get_matrix_winthout_row_and_col(double* buf,  size_t rows_size, size_t cols_size,size_t row, size_t col) const {
	size_t offsetX = 0;
	size_t offsetY = 0;
	...
	for (size_t i = 0; i < rows_size - 1; i++) {
		...
		for (size_t j = 0; j < cols_size - 1; j++) {
			...
			result[i * (cols_size - 1) + j] = buf[cols_size * (i + offsetY) + j + offsetX];
		}
	} 
	return result;
}
...
template<size_t rows, size_t cols>
double Matrix<rows, cols>::get_minor(double *buf,  size_t rows_size, size_t cols_size, size_t row, size_t col) const {
